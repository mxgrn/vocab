defmodule Vocab.DecksTest do
  ...
  describe "decks" do
    ...
    test "create_deck/1 with valid data creates a deck" do
      valid_attrs = %{name: "some name", last_card_inserted_at: ~U[2023-12-06 04:13:00Z]}

      assert {:ok, %Deck{} = deck} = Decks.create_deck(valid_attrs)
      assert deck.name == "some name"
      assert deck.last_card_inserted_at == ~U[2023-12-06 04:13:00Z]
    end
    ...
    test "update_deck/2 with valid data updates the deck" do
      deck = deck_fixture()
      update_attrs = %{name: "some updated name", last_card_inserted_at: ~U[2023-12-07 04:13:00Z]}

      assert {:ok, %Deck{} = deck} = Decks.update_deck(deck, update_attrs)
      assert deck.name == "some updated name"
      assert deck.last_card_inserted_at == ~U[2023-12-07 04:13:00Z]
    end
